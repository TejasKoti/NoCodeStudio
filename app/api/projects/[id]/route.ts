...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ message: status === 401 ? "Unauthorized" : "Server error fetching project" }, { status });
  }
}
...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const status = err?.status || 500;
    return NextResponse.json({ message: status === 401 ? "Unauthorized" : "Server error updating project" }, { status });
  }
}
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const status = err?.status || 500;
    return NextResponse.json({ message: status === 401 ? "Unauthorized" : "Server error deleting project" }, { status });
  }
}