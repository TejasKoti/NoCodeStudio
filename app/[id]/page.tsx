...
function BuilderInner() {
  ...
  const [selectedParams, setSelectedParams] = useState<UIParam[] | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await res.json();
        setCatalog(Array.isArray(data?.layers) ? data.layers : []);
      } catch (err) {
  ...
  const onConnect = useCallback(
    (conn: Connection) => {
      setEdges((eds: RFEdge[]) => addEdge(conn, eds));
      setDirty(true);
    },
    [setEdges]
  );
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      setNodes((nds: RFNode[]) => [...nds, newNode]);
      setDirty(true);
    },
    [setNodes]
  );
  ...
  const layerSchemaCache = useRef<Record<string, any>>({});
  
  const onSelectionChange = useCallback(
    async ({ nodes: selected }: OnSelectionChangeParams) => {
      ...
      const node = getNodeById(selected[0].id);
      if (!node) return;

      setSelectedNode(node);
      setSelectedNodeId(node.id);

      const layerName = node.data.layerName ?? node.data.label;
      const storedParams = node.data.params ?? {};

      let schema = layerSchemaCache.current[layerName];
      if (!schema) {
        const res = await fetch(`/api/layer?name=${encodeURIComponent(layerName)}`);
        if (!res.ok) {
          setSelectedParams([]);
          return;
        }
        schema = await res.json();
        layerSchemaCache.current[layerName] = schema;
      }

      const uiParams = schema.params.map((p: any) => ({
        name: p.name,
        index: p.index,
        required: p.required,
        value:
          storedParams[p.name] !== undefined
            ? String(storedParams[p.name])
            : p.default !== null
            ? String(p.default)
            : "",
      }));

      setSelectedParams(uiParams);
    },
    [getNodeById]
  );
  ...
  const exportCodePY = useCallback(async () => {
    try {
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.download = data.filename || `${projectTitle || "project"}.py`;
      a.href = url;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
  ...
  }, [nodes, edges, projectTitle, token]);
  ...
  const generatePythonCode = async () => {
    ...
    const data = await res.json();
    return data.code || "# No code returned";
  };

  useEffect(() => {
    const missingRequired = nodes.some((n) =>
      Object.values(n.data.params || {}).some(
    ...
    if (missingRequired) {
      setAlertBox({
        type: "red",
        message: "Some layers have required parameters missing.",
      });
    }
  }, [nodes]);
  ...
  const handleRun = useCallback(async () => {
    try {
      ...
      const hasConv = nodes.some(
        (n: RFNode) => n.data?.label?.toLowerCase().includes("conv")
      );
      const hasLinear = nodes.some(
        (n: RFNode) => n.data?.label?.toLowerCase().includes("linear")
      );
      ...
      const data = await res.json();
      const { output, error, stdout } = data;
      ...
      setToast(`Network or backend error: ${err?.message}`), setTimeout(() => setToast(null), 2000);
    } finally {
  ...
  const handleTrain = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      const { error, stdout, metrics } = data;
            ...
            {selectedParams.map((p) => (
              ...
              >
                <div style={{ fontSize: 12, opacity: 0.85 }}>
                  {p.name}
                  {p.required && (!p.value || p.value === "") && (
                    <span style={{ color: "#ff6b6b", marginLeft: 6 }}>*</span>
                  )}
                </div>
                <input
                  value={p.value ?? ""}
        ...
        {["Epochs", "LearningRate", "BatchSize"].map((key) => (
          <div key={key} style={{ marginBottom: 8 }}>
            ...
            <input
              type="number"
              value={
                Number.isFinite(trainerConfig[key as keyof typeof trainerConfig])
                  ? trainerConfig[key as keyof typeof trainerConfig]
                  : ""
              }
              onChange={(e) =>
        ...
        >
          ...
          >
            ...
            <button
              onClick={async () => {
                try {
                  await fetch("/api/cancel_training", {
                    method: "POST",
                    headers: {
                      Authorization: `Bearer ${token}`,
                    },
